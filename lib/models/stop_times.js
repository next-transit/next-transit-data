...
function get_stops(agency_id, route_id, direction_id, from_id, day_of_week, date_str, compare_time, compare_dir, sort_dir, limit, offset) {
  return new promise(function(resolve, reject) {
    ...
    var q = stop_times.select('DISTINCT st.*, t.block_id, tv.stop_count, tv.first_stop_sequence, tv.last_stop_sequence')
      ...
      .where_if('service_id IN (SELECT service_id FROM calendar_dates cd WHERE cd.agency_id = ? AND (cd.exact_date = ? OR (' + day_of_week + ' = true AND cd.start_date <= ? AND cd.end_date > ?)))', [agency_id, date_str, date_str, date_str], date_str)
      .where_if('service_id IN (SELECT service_id FROM calendar_dates cd WHERE cd.agency_id = ? AND ' + day_of_week + ' = true', [], !date_str)
...
stop_times.get_by_time = function(agency, is_rail, route_id, direction_id, from_id, offset) {
  return new promise(function(resolve, reject) {
    var now = date.get_timezone_moment(agency.timezone).subtract(5, 'minutes'),
...
module.exports = exports = stop_times;
